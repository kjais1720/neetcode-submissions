func shortestPathBinaryMatrix(grid [][]int) int {
	visits := make(map[int]map[int]bool)
	...
	for i := 0; i < l; i++ {
		...
		for _, dir := range dirs {
			...
			if _, ok := visits[newr]; ok && visits[newr][newc] {
				continue
			}

			cells = append(cells, []int{newr, newc})
			if _, ok := visits[newr]; ok {
				visits[newr][newc] = true
			} else {
				visits[newr] = map[int]bool{newc: true}
			}
			if !counted {
				length++
				counted = true
